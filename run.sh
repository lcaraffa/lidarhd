...
[ -z "$LIST_FILES" ] || [ -z "$PROJECT_PATH" ] || [ -z "$CENTER_FILE" ] && { echo "Error." ; usage ; }
...
if [ -d "${STATS_DIR}" ]; then
    ...
    log "Start /data/scripts/tile_lidar_files.sh"
...
if [ -d "${CROPED_DIR}" ]; then
    ...
    read -r LAT LONG < ${CENTER_FILE}
    log "Start /data/scripts/crop_lidar_files.sh --input_dir=${RAW_LAZ_DIR} --output_file=${CROPED_DIR}/cropped_merged.laz --LAT=${LAT} --LONG=${LONG} ${BBOX_SIZE_CMD} ${SUBSAMPLE_RATIO_CMD}" "$PROCESS"
    docker run --rm -v ${PROJECT_PATH}:${PROJECT_PATH} \
           -v ${PWD}/scripts:/data/scripts \
           pdal/pdal \
           /bin/bash -c "/data/scripts/crop_lidar_files.sh --input_dir=${RAW_LAZ_DIR} --output_file=${CROPED_DIR}/cropped_merged.laz --LAT=${LAT} --LONG=${LONG} ${BBOX_SIZE_CMD} ${SUBSAMPLE_RATIO_CMD}"
fi
...
PROCESSED_DIR=${PROJECT_PATH}/processed_full
if [ -d "${PROCESSED_DIR}" ]; then
    ...
    log "Start /data/scripts/tile_lidar_files.sh"
    CMD="/data/scripts/tile_lidar_files.sh --input_dir=${INPUT_DIR} --output_dir=${PROCESSED_DIR} --min_x=${MIN_X}  --min_y=${MIN_Y} --pow=4"
    echo "=> $CMD"
    docker run --rm -v ${PROJECT_PATH}:${PROJECT_PATH} \
           -v ${PWD}/scripts:/data/scripts \
           pdal/pdal \
...
./build/lidarhd_env/bin/python3 scripts/convert_ply.py ${PROCESSED_DIR}
